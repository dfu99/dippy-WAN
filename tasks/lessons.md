...
- **Home quota is 20GB**: PACE home dir has a 20GB quota. pip cache (~8GB) and rattler cache (~9GB) can fill it. Clear with `rm -rf ~/.cache/pip`. Put large data on scratch (no quota) not home.
- **p-yke8-0 is deprecated**: All PACE project data now lives at `~/scratch/dippy-WAN/`. The old `~/p-yke8-0/` path is no longer supported. All sbatch scripts use scratch.
...
- **CogVideoX requires Ampere+ GPUs (SM 8.0+)**: The attention layer allocates 113GB for 49 frames at 720x480 without Flash Attention. RTX 6000 (Turing/SM 7.5) doesn't support Flash Attention, so SDPA falls back to naive attention which materializes the full N×N matrix. RTX 3060/3090/A100 (Ampere+) work fine. This is not a VRAM issue — it's a GPU architecture requirement.
...
- **cv2 needed for cache hit frame loading**: When serving from cache, we read the MP4 back into PIL frames via OpenCV (`cv2.VideoCapture`). Make sure `opencv-python` is installed.
